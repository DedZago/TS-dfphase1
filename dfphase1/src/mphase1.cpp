...
namespace {
	...
	inline void ggsubgroupsums(int p, int n, int m, double *x, double *sx) {
		...
		for (i = 0; i < m; i++, sx += p) {
			std::copy(x, x + p, sx);
			for (j = 1, x += p; j < n; j++, x += p) F77_CALL(daxpy)
			(&p, &one, x, &ione, sx, &ione);
		}
	}

	// x[,i] <- x[,i]-m
	inline void ggcenter(int p, int n, double *x, double *m) {
		for (int i = 0; i < n; i++, x += p) F77_CALL(daxpy)
		(&p, &onem, m, &ione, x, &ione);
	}
	...
	void ggmc(int p, int n, int m, double *x, double *l, double *r, double *w) {
		...
		std::fill(r, r + p2, 0.0);
		std::fill(xb, xb + pm, 0.0);
		w = w + pm;
		for (i = 0; i < m; i++, xbi += p)
			for (j = 0; j < n; j++, x += p) ggupdbr(p, j, xbi, r, x, w);
		F77_CALL(dscal)
		(&p2, &sn, r, &ione);
		ggorth(p, m, r, xb, xb);
		ggmmed(p, m, xb, l, w);
	...
	inline void ggforward(int p, int n, int m, double *x, bool isolated, bool step, int lmin, int ncp,
		...
		si = s + p;
		std::fill(s, si, 0.0);
		std::copy(sb, sb + p * m, si);
		for (i = 2, sii = si + p; i <= m; i++, si = sii, sii += p) {
			F77_CALL(daxpy)
			(&p, &one, si, &ione, sii, &ione);
		}
		// main loop
